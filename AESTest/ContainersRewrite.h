...
namespace UC /*UnrealContainers*/
{
	...
	namespace FMemory
	{
		inline void* (*UERealloc)(void* Memory, int64 NewSize, uint32 Alignment) = nullptr; // nullptr safe

		inline void Init(void* R)
		{
			UERealloc = reinterpret_cast<decltype(UERealloc)>(R);
		}

		inline void* Malloc(int32 Size, int32 Align = 0x0)
		{
			return UERealloc(nullptr, Size, Align);
		}

		inline void* Realloc(void* Block, int32 NewSize, int32 Align = 0x0)
		{
			return UERealloc(Block, NewSize, Align);
		}

		inline void* Free(void* Block)
		{
			return UERealloc(Block, 0x0, 0x0);
		}
	}
